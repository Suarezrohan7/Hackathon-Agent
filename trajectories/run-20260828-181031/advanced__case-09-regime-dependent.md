# Trajectory — `advanced/case-09-regime-dependent`

_Rendered 2026-08-28T18:11:08+00:00_

**START** `advanced/case-09-regime-dependent` @ 2026-08-28T18:11:06+00:00

- 💭 profile: {"n_params": 1, "param_names": ["lookback"], "param_grid_size": 3, "reported_is_sharpe": 0.8996, "reported_max_drawdown": -0.1525, "reported_n_trades": 81, "reported_exposure": 0.979, "static_future_reference_flags": []}

- 🧠 **model** (0→0 tok): {"verdict": "inconclusive", "reason": "MOCK response \u2014 set ANTHROPIC_API_KEY for a real run."}

- 🔁 **retry** — safety net: required check 'oos_test' not run by the planner; running it directly

- 🔁 **retry** — safety net: required check 'monte_carlo_permutation' not run by the planner; running it directly

- 🔁 **retry** — safety net: required check 'trade_count' not run by the planner; running it directly

- 💭 verify findings against raw numbers

- 🧠 **model** (0→0 tok): {"verdict": "inconclusive", "reason": "MOCK response \u2014 set ANTHROPIC_API_KEY for a real run."}

- 💭 digest: {"oos_sharpe": -0.5774, "oos_over_is": -0.642, "mc_p_value": 0.4677, "walk_forward_fixed_mean": null, "param_plateau_score": null, "lookahead_suspected": null, "lookahead_static_flags": null, "cost_survives_5bps": null, "n_trades": 81, "checks_run": ["monte_carlo_permutation", "oos_test", "trade_count"]}

- 🧠 **model** (0→0 tok): {"verdict": "inconclusive", "reason": "MOCK response \u2014 set ANTHROPIC_API_KEY for a real run."}


**FINISH** in 2.53s — result: `{"verdict": "inconclusive", "reason": "MOCK response \u2014 set ANTHROPIC_API_KEY for a real run.", "findings": []}`
